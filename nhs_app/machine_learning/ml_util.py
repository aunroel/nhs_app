...
class ML:
    ...
    def __init__(self):
        ...
        self.directory = './models/national'
    ...
    def specify_train(self, train=0.8):
        ...
        if self.national:
            ...
            figure_path = self.directory + "/graphs/" + date_string + "/" + time_string + '.png'
        else:
            self.verify_or_create_dir(f'graphs/{self.postcode}', date_string)
            figure_path = self.directory + '/graphs/' + self.postcode + '/' + date_string + '/' + \
                          time_string + '.png'
    ...
    def define_model(self):
        ...
        #     layers.Dense(len(self.df_data.columns), input_shape=[len(self.train_ds.keys())]),
        #     # usually it's a good idea to have 1 node, which will serve as an output layer
        ...
        # self.model.compile(optimizer=keras.optimizers.SGD(lr=0.1), loss='mean_squared_error', metrics=['mae', 'mse'])
    ...
    def train_model(self):
        ...
        if self.national:
            ...
            figure_path = self.directory + "/results/" + date_string
        else:
            self.verify_or_create_dir(f'results/{self.postcode}', date_string)
            figure_path = self.directory + '/results/' + self.postcode + '/' + date_string

        plt.savefig(figure_path + '/mae_' + time_string + '.png')
        plt.clf()

        plotter.plot({'Early Stopping': early_history}, metric="mse")
        plt.ylim([0, 20])
        plt.ylabel('MSE [MPG^2]')
        plt.savefig(figure_path + '/mse_' + time_string + '.png')
        plt.clf()
    ...
    def predict(self):
        ...
        plt.savefig(figure_path + '/predictions_' + time_string + '.png')
        plt.clf()
        ...
        _ = plt.ylabel("Count")
        plt.savefig(figure_path + '/error_distribution_' + time_string + '.png')
        plt.clf()
    ...
    def save_default_and_convert(self):
        ...
        if self.national:
            ...
            path = self.directory + '/models/' + date_string + '/'
            filename = path + 'default_model_' + \
                       datetime.now().strftime("_%H%M%S")
        else:
            self.verify_or_create_dir('models/' + self.postcode, date_string)
            path = self.directory + '/models/' + self.postcode + '/' + date_string + '/'
            filename = path + 'default_model_' + \
                       datetime.now().strftime("_%H%M%S")
        ...
        if self.national:
            self.verify_or_create_dir('uploaded_lite/', date_string)
            open(self.directory + '/uploaded_lite/' + date_string + '/default_model_' + \
                 datetime.now().strftime("_%H%M%S") + '.tflite', 'wb').write(tflite_model)

            if os.path.exists(self.directory + '/uploaded_lite/default_model.tflite'):
                os.remove(self.directory + '/uploaded_lite/default_model.tflite')
            open(self.directory + '/uploaded_lite/default_model.tflite', 'wb').write(tflite_model)
        else:
            self.verify_or_create_dir('uploaded_lite/' + self.postcode, date_string)
            open(self.directory + f'/uploaded_lite/{self.postcode}/' + date_string + '/default_model_' + \
                 datetime.now().strftime("_%H%M%S") + '.tflite', 'wb').write(tflite_model)

            if os.path.exists(self.directory + '/uploaded_lite/' + self.postcode + '/default_model.tflite'):
                os.remove(self.directory + '/uploaded_lite/' + self.postcode + '/default_model.tflite')
            open(self.directory + '/uploaded_lite/' + self.postcode + '/default_model.tflite', 'wb').write(tflite_model)
